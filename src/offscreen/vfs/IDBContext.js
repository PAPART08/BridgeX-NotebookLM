...
export class IDBContext {
  ...
  async #run(mode, f) {
    ...
    for (let i = 0; i < 2; ++i) {
      let tx = this.#tx;
      if (!tx) {
        tx = this.#tx = db.transaction(db.objectStoreNames, mode, this.#txOptions);
        ...
        this.#putChain = this.#putChain.then(() => {
          return new Promise((resolve, reject) => {
            ...
            tx.addEventListener('abort', event => {
              ...
              reject(e || new DOMException('abort', 'AbortError'));
              if (this.#tx === event.target) {
                this.#tx = null;
              }
      ...
      try {
        const stores = Object.fromEntries(Array.from(db.objectStoreNames, name => {
          return [name, new ObjectStore(tx.objectStore(name))];
        }));
