...
export class IDBMinimalVFS extends VFS.Base {
  ...
  /**
   * @param {string} idbDatabaseName 
   * @param {WebAssembly.Memory} [memory]
   * @param {object} [options]
   */
  constructor(idbDatabaseName, memory, options = DEFAULT_OPTIONS) {
    super();
  ...
  /**
   * Safe helper to re-create a Uint8Array view if the WASM heap grew during an await.
   * Handles both WebAssembly.Memory objects (have .buffer) and raw ArrayBuffers.
   * @param {Uint8Array} pData 
   * @returns {Uint8Array}
   */
  #refresh(pData) {
    if (!this.#memory) return pData;
    
    // Get the current buffer - could be WebAssembly.Memory or ArrayBuffer
    const currentBuffer = this.#memory.buffer || this.#memory;
    
    if (pData.buffer !== currentBuffer) {
      log('Refreshing stale WASM memory view');
      return new Uint8Array(currentBuffer, pData.byteOffset, pData.byteLength);
    }
    return pData;
  }
  ...
  xOpen(name, fileId, flags, pOutFlags) {
    return this.handleAsync(async () => {
      ...
      try {
        ...
        const block0 = await this.#idb.run('readwrite', ({blocks}) => {
          return blocks.get(this.#bound(file, 0));
        });

        if (block0) {
          file.fileSize = block0.data.byteLength;
  ...
  xRead(fileId, pData, iOffset) {
    return this.handleAsync(async () => {
      ...
      try {
        ...
        // REFRESH: Memory might have grown during the await above
        pData = this.#refresh(pData);
  ...
  xWrite(fileId, pData, iOffset) {
    return this.handleAsync(async () => {
      ...
      try {
        // REFRESH: Memory might have grown during worker dispatch OR previous calls
        pData = this.#refresh(pData);

        // Convert the write directly into an IndexedDB object.
        // We MUST use pData reference before any more awaits to be absolutely safe
        const block = {
          path: file.path,
          offset: -iOffset,
          data: pData.slice()
        };
        await this.#idb.run('readwrite', ({blocks}) => blocks.put(block));
