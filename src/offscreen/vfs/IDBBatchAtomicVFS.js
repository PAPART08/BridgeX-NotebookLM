...
export class IDBBatchAtomicVFS extends VFS.Base {
  ...
  xOpen(name, fileId, flags, pOutFlags) {
    return this.handleAsync(async () => {
      ...
      try {
        ...
        // CLEANUP: If opening failed, don't leave a ghost record in the file map
      ...
      try {
        ...
        const result = await this.#idb.run('readonly', async ({blocks}) => {
          ...
          while (pDataOffset < pData.byteLength) {
            ...
            /** @type {FileBlock} */
            const block = (file.block0 && fileOffset < file.block0.data.byteLength) ?
              file.block0 :
              await blocks.get(this.#bound(file, -fileOffset));

            if (!block || !block.data || block.data.byteLength - block.offset <= fileOffset) {
  ...
  #xWriteHelper(fileId, pData, iOffset) {
    ...
    try {
      ...
      if (!file.block0) {
        throw new Error(`xWrite called on uninitialized block0 for ${file.path}`);
      }
    ...
    return this.handleAsync(async () => {
      ...
      try {
        ...
        if (result === VFS.SQLITE_OK && file.locks.state === VFS.SQLITE_LOCK_SHARED) {
          ...
          const newBlock0 = await this.#idb.run('readonly', ({blocks}) => {
            return blocks.get(this.#bound(file, 0));
          });
          if (newBlock0) {
            file.block0 = newBlock0;
          }
        }
        return result;
      } catch (e) {
      ...
      case 33: // SQLITE_FCNTL_ROLLBACK_ATOMIC_WRITE
        return this.handleAsync(async () => {
          try {
            ...
            file.changedPages = null;
            file.isMetadataChanged = false;
            const restoredBlock0 = await this.#idb.run('readonly', ({blocks}) => {
              return blocks.get([file.path, 0, file.block0.version + 1]);
            });
            if (restoredBlock0) {
              file.block0 = restoredBlock0;
            } else {
              // If the old version doesn't exist (e.g. newly created file), we just increment version string
              file.block0.version++;
            }
            return VFS.SQLITE_OK;
          } catch (e) {
  ...
  #bound(file, begin, end = 0) {
    ...
    if (file.block0 && (!begin || -begin < file.block0.data.length)) {
      version = -Infinity;
    } else if (file.block0) {
      version = file.block0.version;
    }
